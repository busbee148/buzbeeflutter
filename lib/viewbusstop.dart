...
import 'package:buzbee/API/busstopAPI.dart';
...
class _BusStopsScreenState extends State<BusStopsScreen>
  ...
  late TabController _tabController;
  late Future<List<dynamic>> busStopsFuture;
  ...
  void initState() {
    super.initState();
    _tabController = TabController(length: 2, vsync: this);
    busStopsFuture = fetchbustop(); // 🟧 API CALL
  }
      ...
      body: FutureBuilder<List<dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError || !snapshot.hasData) {
            return const Center(
              child: Text("Failed to load bus stops",
          ...
          // Nearby ≤ 1.5 km
          final nearbyStops =
              allStops.where((s) => (s['distance'] ?? 0) <= 1.5).toList();

          // Search filter
          final filteredStops = allStops.where((stop) {
            final q = searchQuery.toLowerCase();
            return stop['Stop_name'].toString().toLowerCase().contains(q) ||
                stop['id'].toString().toLowerCase().contains(q) ||
                stop['Landmark'].toString().toLowerCase().contains(q);
          }).toList();
          ...
          return TabBarView(
            ...
            children: [
              ...
              nearbyStops.isEmpty
                  ? const Center(
                      child: Text("No nearby stops within 1.5 km",
                          style: TextStyle(color: Colors.grey)),
                    )
                  : _buildList(nearbyStops),
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    _buildSearchField(),
                    const SizedBox(height: 16),
                    Expanded(child: _buildList(filteredStops)),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildList(List<dynamic> stops) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildStopCard(
          id: s['id'].toString(),
          name: s['Stop_name'] ?? "No Name",
          landmark: s['Landmark'] ?? "No Landmark",
          photo: s['Photo'] ?? "",
          trailing: Text(
            "${s['distance'] ?? 0} km",
            style: const TextStyle(
